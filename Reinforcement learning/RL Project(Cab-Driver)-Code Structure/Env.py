...
class CabDriver():
    ...
    def requests(self, state):
        ...
        if requests == 0:
            requests = 2
        possible_actions_idx = random.sample(range(1, (m-1)*m +1), requests)
    ...
    def time_tracking(self, time, day, ride_duration):
        """
        Tracks time taken after journey
        """
    ...
    def next_state_func(self, state, action, Time_matrix):
        ...
        if ((pickup_loc== 0) and (drop_loc == 0)):
            # Refuse all requests, so wait time is 1, next location is current location
            wait_time = 1
            next_loc = curr_loc
            ...
            new_time, new_day = self.time_tracking(curr_time, curr_day, transit_time)
        ...
        next_time, next_day = self.time_tracking(curr_time, curr_day, total_time)
