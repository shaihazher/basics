...
class TicTacToe():
    ...
    def is_winning(self, curr_state):
        ...
        sum1 = np.any(np.nansum(state, axis=1) == 15.0)
        sum2 = np.any(np.nansum(state, axis=0) == 15.0)
        if sum1:
            row_col_sum = True
        elif sum2:
            row_col_sum = True
        elif np.trace(state) == 15.0:
            row_col_sum = True
        elif np.trace(np.fliplr(state)) == 15.0:
            row_col_sum = True
        else:
            row_col_sum = False
